...
### US-019 – Implementação do Domínio EO (Enterprise Ontology)
```yaml
id: US-019
milestone: R5
prioridade: Alta
tamanho: 13
origem: [RF-16, RF-17]
tags: [type:feature, area:backend, area:eo]
dependencias: []
modulos_afetados: [src/apps/eo]
```

#### Descrição
Criar o novo App Django `eo` e implementar as entidades da Ontologia Empresarial conforme o modelo conceitual.

#### Critérios de Aceitação
- **Funcional**:
    - [ ] App `eo` registrado.
    - [ ] Modelos implementados: `Person`, `TeamMember`, `Team`, `TeamMembership`, `OrganizationalRole`, `TeamPurpose`, `Project`, `Organization`, `ProjectTeam`, `OrganizationalTeam`.
    - [ ] Migrações geradas e aplicadas.
- **Teste (TDD)**:
    - [ ] Testes unitários para criação de cada entidade.
    - [ ] Testes de integridade para relacionamentos (TeamMembership).
- **Design Patterns**:
