...
| **RF-16** | O sistema deve implementar o domínio de Ontologia Empresarial (EO), permitindo a gestão de Organizações, Projetos, Equipes e Papéis. | Entidades base da EO persistidas no banco de dados. | User Req. |
| **RF-17** | O sistema deve suportar a mediação de membros em equipes (Team Membership) vinculada a Papéis Organizacionais e Propósitos da Equipe. | Relacionamentos entre Pessoa, Equipe e Papel gerenciados via Team Membership. | User Req. |
