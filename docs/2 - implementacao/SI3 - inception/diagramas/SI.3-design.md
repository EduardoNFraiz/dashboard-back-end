...
erDiagram
    ...
    %% EO Domain
    TEAM_MEMBERSHIP }o--|| TEAM : "done for"
    TEAM_MEMBERSHIP }o--|| PERSON : "allocates"
    TEAM_MEMBERSHIP }o--|| ORGANIZATIONAL_ROLE : "plays"
    TEAM ||--o{ TEAM_PURPOSE : "has"
    ORGANIZATION ||--o{ TEAM : "has"
    PROJECT ||--o{ TEAM : "has"
```
*Detalhes completos no SI.2 Análise.*

### 4.2 Domínio EO (Enterprise Ontology)
O app `eo` utiliza o Django ORM para persistência em banco de dados relacional (PostgreSQL).

**Entidades Principais:**
- `Person`: Representa um indivíduo.
- `Team`: Unidade coletiva (subclasses via herança do Django ou metadados).
- `TeamMembership`: Entidade associativa (N:N entre Person e Team com metadados e papéis).
- `OrganizationalRole`: Papel desempenhado por um membro.
- `TeamPurpose`: Objetivo da equipe.

**Estrutura do App `src/apps/eo`:**
- `models.py`: Definição de campos, chaves estrangeiras e relacionamentos.
- `admin.py`: Registro dos modelos para gestão via Django Admin.
